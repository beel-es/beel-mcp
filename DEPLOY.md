...
| `MCP_PUBLIC_DISCOVERY` | no | Default `false`. `true` lets a request without a token run `initialize` and the list methods (`tools/list`, `prompts/list`, `resources/list`) on the MCP endpoint, so registries can read the catalogue without an account; `tools/call` still answers 401. Leave it off for hosts that only start the OAuth flow when `initialize` itself is refused. |
