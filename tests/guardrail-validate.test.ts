...
describe('executable guardrails — line pricing', () => {
  ...
  it('rejects a discount on a declared total but allows it on unit_price', () => {
    expect(
      codes({ lines: [{ quantity: 1, total_excluding_tax: 100, discount_percentage: 10 }] }),
    ).toContain('LINE_DECLARED_TOTAL_FORBIDS_DISCOUNT');
...
describe('executable guardrails — equivalence surcharge and regime', () => {
  ...
  it('accepts the coherent pair, and stays silent when regime_key is omitted', () => {
    ...
    const derived = {
      lines: [line({ equivalence_surcharge_rate: 5.2, main_tax: { type: 'IVA', percentage: 21 } })],
    };
...
describe('executable guardrails — invoice level', () => {
  ...
  it('requires a source reference on SUPLIDO lines', () => {
    ...
    expect(
      codes({ lines: [line({ line_type: 'SUPLIDO', source_invoice_reference: 'A/2025/1' })] }),
    ).toEqual([]);
  });

  it('only allows exemption_reason_text under OTRO', () => {
    expect(
      codes({ lines: [line({ exemption_reason: 'EXENTA_ART_21', exemption_reason_text: 'x' })] }),
    ).toContain('EXEMPTION_TEXT_REQUIRES_OTRO');
    expect(
...
describe('executable guardrails — reporting and escape hatch', () => {
  ...
  it('throws a GuardrailError naming the API error code, and honours the escape hatch', () => {
    ...
    expect(() => assertNoViolations(INVOICE_OP, bad, {})).toThrow(
      /LINE_UNIT_PRICE_XOR_DECLARED_TOTAL/,
    );
    expect(() =>
      assertNoViolations(INVOICE_OP, bad, { BEEL_DISABLE_PREFLIGHT: '1' }),
    ).not.toThrow();
  });
});
...
describe('executable guardrails — series numbering', () => {
  ...
  it('requires month and year when the counter resets monthly', () => {
    expect(
      codes({ format: '{CODIGO}-{YYYY}-{NUM:4}', counter_reset: 'MONTHLY' }, SERIES_OP),
    ).toContain('SERIES_MONTHLY_REQUIRES_MONTH_AND_YEAR');
...
describe('every violation carries a usable fix', () => {
  ...
  const PAYLOADS: Array<[string, unknown]> = [
    ...
    [
      'createCompanyInvoice',
      { lines: [{ quantity: 1, unit_price: 10, main_tax: { regime_key: '18' } }] },
    ],
  ...
  it('states the offending value in the message, not just the rule', () => {
    ...
    const irpf = findViolations('createCompanyInvoice', body).find(
      (v) => v.code === 'SIMPLIFICADA_FORBIDS_IRPF',
    );
