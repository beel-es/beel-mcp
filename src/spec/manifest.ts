...
function buildResponseInfo(doc: SpecNode, operation: SpecNode): {
  successContentTypes: string[];
  binaryResponse: boolean;
} {
  ...
  const binaryResponse =
    list.length > 0 &&
    list.every((ct) => BINARY_CONTENT_TYPES.some((bin) => ct.startsWith(bin)));
  ...
  for (const [path, rawItem] of Object.entries(paths)) {
    ...
    for (const method of HTTP_METHODS) {
      ...
      operations.push({
        ...
        path,
        tag: Array.isArray(tags) && typeof tags[0] === 'string' ? tags[0] : 'Other',
