...
/**
 * "…ById" says how the resource is addressed, not what it is. Every tool takes
 * its ids as required inputs, so the suffix distinguishes nothing and is
 * dropped — and with it goes the scope filter for these names, because an
 * operation addressed by id names its subject with the very word that filter
 * removes: `getCompanyById` without both is `get_by_id`, a verb followed by the
 * name of a path parameter.
 */
function stripAddressedById(parts: string[]): string[] | null {
  if (parts.length < 3 || parts.at(-1) !== 'id' || parts.at(-2) !== 'by') return null;
...
export function snakeCase(id: string): string {
  ...
  const parts = joinDomainTerms(words(id));
  const base = stripAddressedById(parts) ?? dropScope(parts);
