...
import { WORKER_TTL } from './constants.js';
...
/**
 * A hostname nobody can prove ownership of.
 *
 * The badge rests on one fact: an authorization code sent to this callback lands
 * in the vendor's hands and nowhere else. That holds only for a name in the
 * public DNS, so every host that resolves inside the user's own machine or
 * network is excluded — `localhost` and its subdomains, the `.local` mDNS space,
 * and every bare IP literal. IP literals go out wholesale rather than by listing
 * the loopback ranges: an address is not a name, so no certificate authority or
 * registrar vouches for who is behind it.
 */
function isUnprovableHost(hostname: string): boolean {
  const host = hostname.toLowerCase();
  if (host === 'localhost' || host.endsWith('.localhost') || host.endsWith('.local')) return true;
  // The URL parser brackets every IPv6 literal and normalises every IPv4 one
  // (including the decimal and hexadecimal spellings) to dotted decimal.
  if (host.startsWith('[')) return true;
  return /^\d{1,3}(\.\d{1,3}){3}$/.test(host);
}

/** A redirect_uri / prefix is verifiable only if it's https on a provable host. */
function isVerifiableCallback(url: string): boolean {
  try {
    const u = new URL(url);
    return u.protocol === 'https:' && !isUnprovableHost(u.hostname);
  } catch {
    return false;
  }
}
 ...
 */
function matchKnownClient(callback: string, knownClients: KnownClient[]): KnownClient | undefined {
...
export async function resolveClientIdentity(
  ...
  const client = await provider.lookupClient(clientId).catch((error: unknown) => {
    // The lookup only supplies the display name and the fallback origin, so a
    // failure degrades the consent screen rather than the flow. It still has to
    // be visible: a lookup that always fails means every client renders nameless
    // and nothing else would say so.
    console.error(`${IDENTITY_MARKER.clientLookupFailed}: ${errorLabel(error)}`);
    return null;
  });
...
/** Stable marker for the one degradation this module can suffer silently. */
export const IDENTITY_MARKER = {
  clientLookupFailed: 'MCP_CLIENT_LOOKUP_FAILED',
} as const;

/** The error's class and message, never anything it may have wrapped. */
...
export const IDENTITY_ASSERTION = {
  ...
  TTL_SECONDS: WORKER_TTL.identityAssertionSeconds,
} as const;
