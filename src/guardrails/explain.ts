 ...
 * a curated subset, because the API puts things there (`defaults_status_endpoint`,
 * `company_id`, `missing_scopes`) more useful than anything written locally. An
 * earlier version filtered them and dropped exactly the field the agent needed.
 *
...
export function explainError(err: ExplainableError): string {
  ...
  if (entry?.actor === 'benign') {
    lines.push(
      '',
      'This is not necessarily a failure — the operation may have already succeeded.',
    );
