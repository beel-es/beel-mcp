 ...
 *
 * It deliberately asks npm nothing about what it packed: `npm pack --json` is a
 * reporting format, free to change shape between npm versions, and a release must
 * not hinge on parsing one. Running the installed binary fails only when a
 * consumer would actually be broken, which is the only signal worth gating on.
 *
 ...
 * so pinning one here would make this check go stale the next time the naming
 * rules change.
 */
const MIN_TOOLS = 100;
